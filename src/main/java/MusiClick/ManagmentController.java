...
public class ManagmentController {
    ...
    private void save_to_BD() {
    	if(inizialize) {
    		...
    		if(songs.size()>0) {
    			//borrar y actualizar
    			ObservableList<Song> aux=SongDAO.getAll();
    			if(aux.size()>0) {
    				for(int i=0;i<aux.size();i++) {
        				if(!songs.contains(aux.get(i))) {
        					SongDAO.delete(aux.get(i));
        				}
        			}
    			}
    		...
    		if(genres.size()>0) {
    			//borrar y actualizar
    			ObservableList<Genre> aux=GenreDAO.getAll();
    			if(aux.size()>0) {
    				for(int i=0;i<aux.size();i++) {
        				if(!genres.contains(aux.get(i))) {
        					GenreDAO.delete(aux.get(i));
        				}
        			}
    			}
    		...
    		if(artists.size()>0) {
    			//borrar y actualizar
    			ObservableList<Artist> aux=ArtistDAO.getAll();
    			if(aux.size()>0) {
    				for(int i=0;i<aux.size();i++) {
        				if(!artists.contains(aux.get(i))) {
        					ArtistDAO.delete(aux.get(i));
        				}
        			}
    			}
    ...
    private void show_Alert (int type) {
    	String f="";
    	switch(type) {
    	case 0: f="Necesita cargar los datos de la BD.";
    			break;
    	case 1: f="Operación incorrecta, este id ya está en uso.";
    			break;
    	case 3: f="No hay datos para operar.";
    			break;
    	case 4: f="Datos cargados correctamente.";
				break;
    	case 5: f="Se ha actualizado su base de datos correctamente";
				break;
    	}
    ...
    private void add_Artist() {
    	if(inizialize) {
    		if(txt_artist_id.getText().matches("[0-9]+")
    				&&!txt_artist_name.getText().matches("")) {
    		    		...
    		    		if(!same_Id) {
    		    			artist=new Artist(Integer.parseInt(txt_artist_id.getText()+""),txt_artist_name.getText(),txt_artist_photo.getText());
    		    			artists.add(artist);
    ...
    private void edit_Artist() {
    	try {
    		if(
        			artist!=null
        			&&!txt_artist_id.getText().matches("")
        			&&txt_artist_id.getText().matches("[0-9]+")
        			&&!txt_artist_name.getText().matches("")
    ...
    private void add_Genre() {
    	if(inizialize) {
    		if(txt_genre_id.getText().matches("[0-9]+")
    ...
    private void edit_Genre() {
    	...
    	if(
    			genre!=null
    			&&!txt_genre_id.getText().matches("")
    			&&txt_genre_id.getText().matches("[0-9]+")
    			&&!txt_genre_name.getText().matches("")) {
    ...
    private void add_Song() {
    	if(inizialize) {
    		if(!txt_song_id.getText().matches("")
        			&&txt_song_id.getText().matches("[0-9]+")
        			&&!txt_song_name.getText().matches("")
        			&&!txt_song_photo.getText().matches("")
        			&&!txt_song_media.getText().matches("")
        			&&!txt_song_duration.getText().matches("")
        			&&(txt_song_duration.getText().matches("[0-9]+"+"\\."+"[0-9]{1,}")|txt_song_duration.getText().matches("[0-9]+"))
	    		...
	    		if(!same_Id) {
	    			...
	    			song=new Song(
	    					Integer.parseInt(txt_song_id.getText()+""),
	    					txt_song_name.getText(),
	    					txt_song_photo.getText(),
	    					txt_song_media.getText(),
	    					com_song_artist.getSelectionModel().getSelectedItem(),			
	    					Double.parseDouble(txt_song_duration.getText()),
	    					0,
    ...
    private void edit_Song() {
    	...
    	if(
    		...
    		if(!same_Id|Integer.parseInt(txt_song_id.getText())==song.getId()) {
        		...
        		table_song.getSelectionModel().getSelectedItem().setDuration(Double.parseDouble(txt_song_duration.getText()));
        		table_song.getSelectionModel().getSelectedItem().setArtist(com_song_artist.getSelectionModel().getSelectedItem());
        		table_song.getSelectionModel().getSelectedItem().setGenre(com_song_genre.getSelectionModel().getSelectedItem());
        		table_song.getSelectionModel().getSelectedItem().setDisc(com_song_disc.getSelectionModel().getSelectedItem());
